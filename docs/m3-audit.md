...
- `home_cards.dart` 的 Hero 渐变仍是 6 个硬编码色值（亮暗各 3），未接入主题令牌。
  改成令牌需要先在配置里定义一组渐变角色，属于设计决策而非合规修复。
...
| 13 | 移动端底部 `NavigationBar` 放了 **6 个**目的地，超出 M3 建议的 3–5 | **未修**：属产品决策，见下 |
...
**关于 #13**：`AppSection` 有 6 个分区（首页 / 搜索 / 音乐库 / 收藏 / 播放列表 / 设置），
在 compact 尺寸下全部塞进底部导航栏。M3 的建议是 **3–5 个目的地**，超出后标签会挤压。
可行的处理方向：把「收藏」并入「音乐库」作为一个分段，或把「设置」移到顶部的溢出菜单。
这需要产品决策，不擅自改。
...
> 以下为**初评时**的排序，保留作为决策依据的记录。第 1–6 项已于 2026-09-16 全部完成，
> 详见开头「修复记录」。第 7 项部分完成。
